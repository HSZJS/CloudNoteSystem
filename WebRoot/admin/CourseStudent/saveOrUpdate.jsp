...
<html>
	...
	<body>
		...
		<form
			action="${pageContext.request.contextPath}/CourseStudent-saveOrUpdateObject.action"
			method="post">
			...
			<table
				...
				<tbody>
					<tr>
						...
						<td>
							<select name="util.course.id">
								<c:forEach var="type" items="${listCourse}">
									<option
										<c:if test="${type.id == util.course.id }">
																	selected
																	</c:if>
										value="${type.id }">
										${type.s_0 }
