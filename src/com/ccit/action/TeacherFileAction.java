...
public class TeacherFileAction extends ActionSupport {
	...
	private TeacherFile util;
	private List<TeacherFile> list;
	...
	public String getAllUtil() throws Exception {
		...
		if (null != message && message.trim().length() > 0) {
			...
			str = _Str;
			sql = "from TeacherFile where " + message + " like '%" + str + "%'";
		} else {
			sql = "from TeacherFile";
		}
		message = null;
	...
	public String updateUtil() throws Exception {
		...
		TeacherFile tmp = (TeacherFile) objectManager.getObject(
				new TeacherFile(), util.getId());
		if (fileName == null
	...
	public String selectUtil() throws Exception {
		listUser = objectManager.getUtil("from User");
		util = (TeacherFile) objectManager.getObject(new TeacherFile(), util
				.getId());
		return "saveOrUpdate";
	}
	...
	public List<TeacherFile> getList() {
		return list;
	}

	public void setList(List<TeacherFile> list) {
		this.list = list;
	}
	...
	public TeacherFile getUtil() {
		return util;
	}

	public void setUtil(TeacherFile util) {
		this.util = util;
	}
